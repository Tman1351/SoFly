...
export interface IphoneWithContentProps extends HTMLAttributes<HTMLDivElement> {
  children: ReactNode
}

export function IphoneWithContent({
  children,
  className,
  style,
  ...props
}: IphoneWithContentProps) {
  const maskId = useId();
  const uniqueMaskId = `screenPunch-${maskId}`;
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute z-10 overflow-hidden bg-transparent dark:bg-gray-900"
        style={{
          left: `${LEFT_PCT}%`,
          top: `${TOP_PCT}%`,
          width: `${WIDTH_PCT}%`,
          ...
          clipPath: `inset(0 round ${(SCREEN_RADIUS / SCREEN_WIDTH) * 100}% / ${(SCREEN_RADIUS / SCREEN_HEIGHT) * 100}%)`,
        }}
      >
        {children}
      </div>
      ...
      >
        ...
        <g mask={`url(#${uniqueMaskId})`}>
          ...
          <path
            ...
            className="dark:fill-black"
          />
          {/* Inner frame border - slightly lighter for definition */}
          <path
        ...
        {/* Screen border - subtle dark stroke */}
        <path
        ...
        {/* Camera and sensors - Black with subtle detail */}
        <path
